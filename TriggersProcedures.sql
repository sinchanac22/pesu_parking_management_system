...
CREATE FUNCTION p.calculate_amount_to_be_paid(
...
DETERMINISTIC
BEGIN
    DECLARE extra_charge DECIMAL(10,2);
    DECLARE time_difference DECIMAL(10,2);
    DECLARE amount_to_be_paid DECIMAL(10,2);
    
    SET extra_charge = 0;
    
    IF payment_datetime IS NOT NULL THEN
        SET time_difference = TIMESTAMPDIFF(HOUR, payment_datetime, reservation_datetime);
        
        IF time_difference >= 720 THEN        -- 1 month
            SET extra_charge = 15;
        ELSEIF time_difference >= 168 THEN    -- 1 week
            SET extra_charge = 10;
        ELSEIF time_difference >= 24 THEN     -- 1 day
            SET extra_charge = 5;
        END IF;
    END IF;
    
    -- Calculate base amount using the same logic as Python function
    IF duration < 24 THEN
        SET amount_to_be_paid = hourly_rate * duration;
    ELSEIF duration < 720 THEN
        SET amount_to_be_paid = daily_rate * (duration DIV 24);
    ELSE
        SET amount_to_be_paid = monthly_rate * (duration DIV (24 * 30));
    END IF;
    
    SET amount_to_be_paid = amount_to_be_paid + extra_charge;
