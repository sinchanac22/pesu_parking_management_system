...
# Establish a connection to the MySQL database with timeout settings
connection = pymysql.connect(
    host="localhost", 
    user="root", 
    password="Sinchana@2004", 
    database="p", 
    connect_timeout=600  # Set timeout to 10 minutes
)
...
SENDER_EMAIL = 'sinchanachandranaik2004@gmail.com'
SENDER_PASSWORD = 'ahus ytph fuvt gaul'
...
def send_otp(receiver_email):
    try:
        # Only generate and store OTP if it's not already set
        if 'otp' not in st.session_state or st.session_state.otp is None:
            new_otp = generate_otp()
            st.session_state.otp = new_otp  # Store OTP in session state
            
            # Create email message
            message = MIMEMultipart()
            message['From'] = SENDER_EMAIL  # Set your email
            ...
            with smtplib.SMTP('smtp.gmail.com', 587) as server:
                server.starttls()
                server.login(SENDER_EMAIL, SENDER_PASSWORD)  # Add your email credentials
                server.sendmail(SENDER_EMAIL, receiver_email, message.as_string())
            
            st.success("An OTP has been sent to your email!")
        else:
            st.info("OTP already sent. Check your email.")
        
        return True

    except Exception as e:
        st.error(f"Failed to send OTP: {str(e)}")
        st.session_state.otp = None  # Clear OTP on failure
        return False

# Function to handle parking reservation
def reserve_parking_space(user_id):
    # Simulate fetching the user email from a database
    ...
    # If OTP is not verified yet, handle OTP sending and verification
    if not st.session_state.otp_verified:
        # Send OTP only if it hasn't been sent yet
        if st.session_state.otp is None:
            send_otp(email)  # Only send OTP once, with only email argument

        # Get user input for OTP verification
        user_otp = st.text_input("Enter the OTP sent to your email:")

        if st.button("Proceed!!!"):
            # Debugging: show OTP values
            st.write(f"Debugging: OTP from session: {st.session_state.otp}")
            st.write(f"Debugging: OTP entered by user: {user_otp}")

            # Validate the entered OTP
            if user_otp.strip() == str(st.session_state.otp).strip():
                st.session_state.otp_verified = True
                st.success("OTP verified successfully!")
                # Clear OTP after successful verification
                st.session_state.otp = None
                st.experimental_rerun()  # Rerun the app to refresh UI
            else:
                st.error("Invalid OTP. Please try again.")

        # Optionally allow resending OTP if necessary
        if st.button("Resend OTP"):
            # Reset OTP and resend
            st.session_state.otp = None
            send_otp(email)

    # If OTP is verified, show the reservation form and active reservations
    if st.session_state.otp_verified:
        st.write("OTP verified! Proceed with reservation...")

        # Fetch active reservations after OTP is verified
        get_active_reservations(user_id)

        # Optionally reset OTP verification for the next session
        st.session_state.otp_verified = False
...
def get_active_reservations(user_id):
    ...
    if cursor.rowcount > 0:
        ...
        reservations_df = pd.DataFrame(
            reservation_data,
            columns=[ 
                "ReservationID", "SpaceType", "ReservationDateTime", "Duration", "RegistrationNumber", 
                "VehicleType", "AreaName", "Location", "HourlyRate", "DailyRate", "MonthlyRate", 
